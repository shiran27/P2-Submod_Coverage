...
% NSF under grants ECCS-1931600, DMS-1664644, CNS-1645681, by AFOSR under grant FA9550-19-1-0158, by ARPA-E under grant DE-AR0001282 and by the NEXTCAR program under grant DE-AR0000796 and by the MathWorks.
}
...
\begin{align}
    ...
    \quad \quad \quad \quad \quad \quad \quad \, \mbox{\color{blue} (using \eqref{Eq:MarginalGainNotation})} \nonumber \\
    ...
    \quad \quad \quad \quad \, \mbox{\color{blue} (using \eqref{Eq:MarginalGainNotation})}  \nonumber \\
    =& \Delta f(Y^G_n \vert Z^{nN}) + \Delta f(Y^*_n \vert Z^{(n+1)N}) 
    \quad \quad \quad \quad \ \mbox{(using \eqref{Eq:GreedySolutionBlock})} \nonumber\\
    \leq& \Delta f(Y^G_n \vert Z^{nN}) + \underset{Y \in \mathcal{I}^N_{n}}{\max}\ \Delta f(Y \vert Z^{(n+1)N}) 
    \quad \ \, \mbox{\color{blue} (as $Y^*_n \in \mathcal{I}^N_n$)} \nonumber\\
    =& \Delta f(Y^G_n \vert Z^{nN}) + \underset{Y \in \mathcal{I}^N_{n+1}}{\max}\ \Delta f(Y \vert Z^{(n+1)N}) 
    \quad \mbox{\color{blue}(From Lm. \ref{Lm:MaxTransfer})} \nonumber\\
    =& \Delta f(Y^G_n \vert Z^{nN}) + \Delta f(Y^*_{n+1} \vert Z^{(n+1)N}). 
    \quad \quad \quad \ \mbox{\color{blue} (using \eqref{Eq:AuxiliaryProblem})} \nonumber
\end{align}
...
Therefore, $\beta_u$ seems to have the best of both worlds while also being computationally inexpensive and having no additional technical limitations on its applicability. In the next section, we confirm these conclusions using numerical results generated from several different experiments. 
...
\begin{figure}[!b]
    ...
    \begin{subfigure}[t]{\columnwidth}
        ...
        \caption{\color{blue} $\lambda=0.006, N=10$ \\ Average Improvement = $0.1591$}
    \end{subfigure}%
    \\
    \centering
...
\begin{figure}[!b]
    ...
    \begin{subfigure}[t]{\columnwidth}
        ...
        \caption{\color{blue} $\delta=400, N=10$ \\ Average Improvement = $0.1386$}
    \end{subfigure}%
    \\
    \centering
    \begin{subfigure}[t]{0.105\textwidth}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[t]{0.33\textwidth}
        ...
        \caption{\color{blue} $\delta = 200, \lambda = 0.008$ \\ Average Improvement = $0.1855$}
    \end{subfigure}%
    \hfill
    \begin{subfigure}[t]{0.33\textwidth}
        \centering
        ...
        \caption{\color{blue} $\delta = 300, \lambda = 0.006$ \\ Average Improvement = $0.1856$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.33\textwidth}
        \centering
        ...
        \caption{\color{blue} $\delta = 400, \lambda = 0.004$ \\ Average Improvement = $0.2106$}
    \end{subfigure}
    \\
    \centering
...
\begin{figure}[!t]
    ...
    \begin{subfigure}[t]{\columnwidth}
        ...
        \caption{\color{blue} $\delta=800, N=10$ \\ Average Improvement = $0.1248$}
    \end{subfigure}%
    \\
    \centering
    \begin{subfigure}[t]{0.105\textwidth}
    ...
    \end{subfigure}
    \caption{\color{blue} Performance Bound vs. Sensing Decay Rate ($\lambda$) for the Blank mission space configuration. Sub-figures (b)-(d) show three greedy solutions obtained for three different $\lambda$ values.}
...
% two obstacle configurations has been considered
% covered red yello lightly coloed Green obstacles
...
% the dip in the $\beta_u$ curve seen in each of the Figs. \ref{Fig:GeneralConfigBetaVsRange}, \ref{Fig:GeneralConfigBetaVsDecay}, \ref{Fig:MazeConfigBetaVsN}(a), \ref{Fig:MazeConfigBetaVsN}(b) and \ref{Fig:MazeConfigBetaVsN}(c)) also point out that using a greedy algorithm to solve a multi-agent coverage problem is most challenging when the agents have a moderate sensing capability. Hence, future research is directed at improving the greedy algorithm and the performance bounds to address such challenging scenarios arising in submodular maximization problems. 
